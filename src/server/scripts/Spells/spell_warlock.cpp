...
public:
	...
	class spell_warl_fury_ward_AuraScript : public AuraScript
	{
		...
		bool CalculateAbsorb(constAuraEffectPtr /*aurEff*/, int32 & amount, bool & /*canBeRecalculated*/)
		{
			if (Unit* caster = GetCaster())
	...
	class spell_warl_dark_apotheosis_AuraScript : public AuraScript
	{
		...
		void OnApply(constAuraEffectPtr /*aurEff*/, AuraEffectHandleModes /*mode*/)
		{
			if (Unit* caster = GetCaster())
			{
				...
				if (AuraPtr s12 = caster->AddAura(WARLOCK_ITEM_S12_TIER_4, caster))
		...
		void HandleAfterHit()
		{
			if (Player* _player = GetCaster()->ToPlayer())
			{
				if (Unit* target = GetHitUnit())
				{
					...
					{
						if (AuraPtr hauntAura = target->GetAura(48181, _player->GetGUID()))
						{
	...
	class spell_warl_chaos_bolt_SpellScript : public SpellScript
	{
		...
		void HandleAfterCast()
		{
			if (Unit* l_Caster = GetCaster())
			{
				...
				{
					if (AuraPtr l_Backdraft = l_Caster->GetAura(WarlockSpells::WARLOCK_BACKDRAFT))
					{
...
public:
	...
	class spell_warl_ember_tap_glyph_AuraScript : public AuraScript
	{
		...
		bool CalculateAmount(constAuraEffectPtr, int32& p_Amount, bool& /*canBeRecalculated*/)
		{
			Unit* l_Caster = GetCaster();
	...
	class spell_warl_ember_tap_glyph_SpellScript : public SpellScript
	{
		...
		void HandleBeforeHit()
		{
			...
			if (AuraEffectPtr l_PreviousEmberTap = l_Caster->GetAuraEffect(l_SpellInfo->Id, EFFECT_2, l_Caster->GetGUID()))
			{
				int32 l_Duration = 0;
				...
				if (AuraPtr l_EmberTap = l_Caster->GetAura(GetSpellInfo()->Id))
				{
		...
		void HandleAfterHit()
		{
			...
			AuraPtr l_Aura = l_Caster->GetAura(l_SpellInfo->Id);
			if (!l_Aura)
				return;
			...
			if (AuraEffectPtr l_AuraEffect = l_Caster->GetAuraEffect(l_SpellInfo->Id, EFFECT_2))
			{
	...
	class spell_warl_siphon_life_AuraScript : public AuraScript
	{
		...
		void OnTick(constAuraEffectPtr aurEff)
		{
			if (Unit* caster = GetCaster())
			{
				if (caster->IsAlive())
				{
					if (AuraEffectPtr siphonLifeEffect = caster->GetAuraEffect(WARLOCK_GLYPH_OF_SIPHON_LIFE, EFFECT_0))
					{
...
public:
	...
	class spell_warl_soul_fire_SpellScript : public SpellScript
	{
		...
		void HandleOnCast()
		{
			if (Unit* l_Caster = GetCaster())
			{
				if (AuraPtr l_MoltenCore = l_Caster->GetAura(WARLOCK_MOLTEN_CORE))
